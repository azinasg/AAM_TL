...
#=======================================================================================================================
def Compute_Shape_Logistic_Weight(S_Data,T_Data,S_mean,T_mean,S_Comp,T_Comp):
    # Building Data and Lables for Logistic Regression
    data = np.vstack((S_Data,T_Data))
    Proj_S = np.dot(data-S_mean,S_Comp.T)
    Proj_T = np.dot(data-T_mean,T_Comp.T)
    ...
    LRC = LogisticRegressionCV(Cs=[0.1,1,10,100,1000,10000], fit_intercept=True, class_weight='balanced', cv=5,
                               ...
                               n_jobs=1, verbose=1, refit=True, intercept_scaling=1.0, multi_class='ovr',
                               random_state=None)
    LRC.fit(Data,Lables)
...
#=======================================================================================================================
def Compute_App_Logistic_Weight(S_Data,T_Data,S_Comp,T_Comp,pwa_sut_t,pwa_sut_s,SUT_tmp,S_ref,T_ref,S_mean,T_mean):
    data = np.vstack((S_Data,T_Data))
    Warped_SUT_to_T = []
    for i in range(0, data.shape[0]):
        img = SUT_tmp.from_vector(data[i])
        warped = img.as_unmasked(copy=False).warp_to_mask(T_ref.mask, pwa_sut_t)
        Warped_SUT_to_T.append(warped)

    data_T = as_matrix(Warped_SUT_to_T, return_template=False, verbose=True)

    Warped_SUT_to_S = []
    for i in range(0, data.shape[0]):
        img = SUT_tmp.from_vector(data[i])
        warped = img.as_unmasked(copy=False).warp_to_mask(S_ref.mask, pwa_sut_s)
        Warped_SUT_to_S.append(warped)

    data_S = as_matrix(Warped_SUT_to_S, return_template=False, verbose=True)

    Proj_S = np.dot(data_S - S_mean,S_Comp.T)
    Proj_T = np.dot(data_T - T_mean,T_Comp.T)
    Data = np.hstack((Proj_S,Proj_T))
    Lables = np.hstack((np.ones(S_Data.shape[0]),np.zeros(T_Data.shape[0])))
...
for x in range(0,11,1):
    ...
    Source_weights = Compute_Shape_Logistic_Weight(SS_data, ST_data, SS_mean, ST_mean,
    ...
    # Defining Source Weight Vector
    Source_App_weights = Compute_App_Logistic_Weight(SS_App_data,ST_App_data,source_aam.appearance_models[0]._components,
                                                     target_aam.appearance_models[0]._components, pwa_sut_t, pwa_sut_s,
                                                     SUT_App_template, S_reference_frame,
                                                     T_reference_frame, source_aam.appearance_models[0]._mean,
    ...
    for k,i in enumerate(test_images):
        ...
        for j in range(0, 10):
            ...
            print "Ins_Log_WL : alpha=" + str(alpha) + " beta=" + str(beta)+" k=" + str(k) + " j=" + str(j) + \
                  " initial err: " + str(fr.initial_error()) + " final err: " + str(fr.final_error())

    with open(r'/u/azinasg/res16/Instance_Logistic_WL_a='+str(alpha)+'.pkl', 'wb') as f:
