...
for x in range(3,8,1):
    ...
    ST = rescale_images_to_reference_shape(target_images, 'face_ibug_66_trimesh' ,
                                           target_aam.reference_shape, verbose=True)
    SS = rescale_images_to_reference_shape(source_images, 'face_ibug_66_trimesh' ,
                                           source_aam.reference_shape, verbose=True)
    ...
    SS_weights = np.sqrt(Source_weights)
    WSS = np.diag(SS_weights)
    SS_data -= SS_mean
    SS_data = WSS.dot(SS_data)

    # Buidling Data for PCA
    tmp_e_vectors, tmp_e_values, tmp_mean = pca(SS_data, centre=False, inplace=True)
    source_aam.shape_models[0].model._components = tmp_e_vectors
    source_aam.shape_models[0].model._eigenvalues = tmp_e_values
    ...
    # ==================================================================================================================
    Shape_e_vectors = np.vstack((target_aam.shape_models[0].model._components,
    ...
    SS_App_weights = np.sqrt(Source_App_weights)
    ...
    tmp_App_e_vectors, tmp_App_e_values, tmp_App_e_mean = pca(SS_App_data, centre=False, inplace=False)
    ...
    Warped_T_to_SUT = []
    for i in range(0, target_aam.appearance_models[0]._components.shape[0]):
        img = T_reference_frame.from_vector(target_aam.appearance_models[0]._components[i, :])
        warped = img.as_unmasked(copy=False).warp_to_mask(SUT_reference_frame.mask, pwa_t_sut)
        Warped_T_to_SUT.append(warped)
    new_target_app_evectors = as_matrix(Warped_T_to_SUT, return_template=False, verbose=True)

    # Concatenating Target and Discarded Source Appearance Space
    App_e_vectors = np.vstack((new_target_app_evectors, tmp_App_e_vectors))
    App_e_values = np.concatenate((target_aam.appearance_models[0]._eigenvalues, tmp_App_e_values))
    App_n_new_samples = len(target_aam.appearance_models[0]._eigenvalues) + len(tmp_App_e_values)
    ...
    for k,i in enumerate(test_images):
        ...
        for j in range(0, 10):
            ...
            print "Ins_Heu_WN : alpha=" + str(alpha) + " k=" + str(k) + " j=" + str(j) + " initial err: " + \
                  str(fr.initial_error()) + " final err: " + str(fr.final_error())

    with open(r'/u/azinasg/res16/Azin_a='+str(alpha)+'.pkl', 'wb') as f:
