...
#=======================================================================================================================
Shape_e_vectors = np.vstack((target_aam.shape_models[0].model._components,New_Shape_Components))
Shape_e_values = np.concatenate((target_aam.shape_models[0].model._eigenvalues,New_Shape_Eigen_Values))
n_new_samples = ST_N + len(New_Shape_Eigen_Values)

#=======================================================================================================================
### Updating Shape Components and Eigen Values ###
#=======================================================================================================================
Shape_tmp = PCAModel.init_from_components(components=Shape_e_vectors, eigenvalues=Shape_e_values,
                                          mean=target_aam.shape_models[0].model.mean(),
                                          n_samples = n_new_samples, centred=True,
                                          max_n_components=target_aam.max_shape_components[0])
# Setting Models info
target_aam.shape_models[0].model = Shape_tmp

target_aam.shape_models[0]._target = None

target_aam.shape_models[0]._weights = np.zeros(target_aam.shape_models[0].model.n_active_components)

target_aam.shape_models[0]._target = target_aam.shape_models[0].model.mean()

shape_mean = target_aam.shape_models[0].model.mean()

target_aam.shape_models[0].global_transform = DifferentiableAlignmentSimilarity(shape_mean,shape_mean)
# Re-orthonormalize components against similarity transfrom
target_aam.shape_models[0]._construct_similarity_model()
# Reset the target given the new model
target_aam.shape_models[0]._sync_target_from_state()
...
App_e_vectors = np.vstack((target_aam.appearance_models[0]._components, New_Appearance_Components))
...
#=======================================================================================================================
### Updating Appearance Components and Eigen Values
#=======================================================================================================================
reset = (target_aam.appearance_models[0].n_active_components == target_aam.appearance_models[0].n_components)
...
App_tmp = PCAModel.init_from_components(components=App_e_vectors, eigenvalues=App_e_values,
                                          mean=target_aam.appearance_models[0].mean(),
                                          n_samples = App_n_new_samples,centred=True,
                                         max_n_components=target_aam.max_appearance_components[0])

del target_aam.appearance_models[0]
target_aam.appearance_models.append(App_tmp)
...
for x in range(len(alpha)):
    ...
    for k,i in enumerate(test_images):
        ...
        for j in range(0, 10):
            ...
            print "T_Dis : A=" + str(alpha[x]) + " B=" + str(beta[x]) + " k=" + str(k) + " j=" + str(j) + \
                  " initial err: " + str(fr.initial_error()) + " final err: " + str(fr.final_error())

    with open(r'/u/azinasg/res16/T_Discarded_Source_x='+str(x)+'.pkl', 'wb') as f:
